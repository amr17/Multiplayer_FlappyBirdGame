...
def main():
    ...
    IMAGES['DRAW']= pygame.image.load('assets/sprites/DRAW.png').convert_alpha()
    ...
    while True:
        for event in pygame.event.get():
            ...
            if keys[K_t]:
                ...
                print(TCPportNumber)
                data = data.decode()
                if data=="LET's PLAY TOGETHER":
                    ackMessage = "LETS GET STARTED"
                    time.sleep(1)
...
def mainGame(movementInfo):
    ...
    while True:
            if time.clock()-startTime<30:
                ...
                ShowTime(30-int(time.clock() - startTime))
...
def ShowTime(time):
    """displays score in center of screen"""
    scoreDigits = [int(x) for x in list(str(time))]
    totalWidth = 0  # total width of all numbers to be printed

    for digit in scoreDigits:
        totalWidth += IMAGES['numbers'][digit].get_width()

    Xoffset = (SCREENWIDTH - totalWidth) / 1.15

    for digit in scoreDigits:
        SCREEN.blit(IMAGES['numbers'][digit], (Xoffset, SCREENHEIGHT * 0.05))
        Xoffset += IMAGES['numbers'][digit].get_width()

def showOppnentScore(score):
    """displays score in center of screen"""
    scoreDigits = [int(x) for x in list(str(score))]
    totalWidth = 0  # total width of all numbers to be printed
